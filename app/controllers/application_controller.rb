class ApplicationController < ActionController::Base
  ...
  # The navigation UI element (1 Select Works and etc) will not be displayed on the static pages
  ...
  # prevent the user from moving forward if the work limit has been reached
  def work_limit_reached?
    return false if @cart.size < ENV["WORK_LIMIT"].to_i

    flash[:alert] = "The maximum report size is #{ENV["WORK_LIMIT"].to_i} " \
                    "works. Go to <a href=\"#{preview_path}\">Preview List</a> " \
  ...
  # Sets fields used by the UI for results paging of works.
